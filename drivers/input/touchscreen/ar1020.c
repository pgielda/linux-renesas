...
#ifdef CONFIG_TOUCHSCREEN_AR1020_ANDROID_CALIBRATION
static int android_calibration[7] = {1, 0, 0, 0, 1, 0, 1};

module_param_array_named(calibration, android_calibration, int, NULL, 0644);
MODULE_PARM_DESC(calibration, "android calibration data");
#endif
...
static void ar1020_handle_touch(struct ar1020_priv *priv)
{
	...
	priv->last_x = (priv->touch.buf[1] & 0x7F) | ((priv->touch.buf[2] & 0x1F) << 8);
	priv->last_y = (priv->touch.buf[3] & 0x7F) | ((priv->touch.buf[4] & 0x1F) << 8);

	input_report_key(priv->input, BTN_TOUCH, !!pen_down);
#ifndef CONFIG_TOUCHSCREEN_AR1020_ANDROID_CALIBRATION
	...
	input_report_abs(priv->input, ABS_X,
			 (android_calibration[3] * priv->last_x +
			  android_calibration[4] * priv->last_y +
			  android_calibration[5]) /
			 android_calibration[6]);
	input_report_abs(priv->input, ABS_Y,
			 (android_calibration[3] * priv->last_y +
			  android_calibration[4] * priv->last_x +
			  android_calibration[5]) /
			 android_calibration[6]);
#endif			 
			...
			const struct i2c_device_id *did)
{
	...
	priv->input->absbit[0] = BIT(ABS_X) | BIT(ABS_Y) | BIT(ABS_PRESSURE);
 
	priv->input->open   = ar1020_open;
	priv->input->close  = ar1020_close;

	input_set_abs_params(priv->input, ABS_X, 0, 1<<13, 0, 0);
	input_set_abs_params(priv->input, ABS_Y, 0, 1<<13, 0, 0);
